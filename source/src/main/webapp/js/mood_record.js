...
//気分とコメントが未入力だった場合
	...
	const errors = [];//エラーメッセージ追加用の箱

		if (!mood) {//気分選択されてないとき
		  	  errors.push("気分登録は必須です");
		}
		if (comment.length > 140) {//コメントが140字以上の時（141字からエラー表示）
		  	  errors.push("140文字以内で入力してください。");
		}
		if (errors.length > 0) {//エラーの表示方法
			  errorMessageDiv.innerHTML = errors.join("<br>"); // 改行して複数表示
			  e.preventDefault(); // 送信キャンセル
		  return;//１個でもエラーがあればフォーム送信停止
		}
 ...
 //-----------ログをその場で画面上に追加（上に表示）------------------//
    ...
    const logDiv = document.createElement("div");//divクラス作成
	      ...
	      if (moodLogList) {
		        ...
		        //insertBefore = どこに追加するか指定
		        //logDiv, moodLogList.firstChild ＝ logDivを一番上に追加
	      }
    });